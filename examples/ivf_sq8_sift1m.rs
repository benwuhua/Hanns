//! IVF-SQ8 SIFT-1M authority benchmark
//! Usage:
//!   cargo run --example ivf_sq8_sift1m --release -- <data_dir>
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() < 2 {
        eprintln!("Usage: cargo run --example ivf_sq8_sift1m --release -- <data_dir>");
        std::process::exit(2);
    }
    ...
    println!("=== IVF-SQ8 SIFT-1M authority benchmark ===");
    ...
    #[cfg(feature = "parallel")]
    {
        ...
        let batch = index.search_parallel(&queries, &req, 0)?;
        ...
        for i in 0..recall_queries {
            let start = i * TOP_K;
            let end = start + TOP_K;
            recall_sum += recall_at_k(&gt[i], &batch.ids[start..end], TOP_K);
        }
        let recall = recall_sum / recall_queries as f64;
