...
#[derive(Debug, Clone)]
pub struct IvfUsqSectionedExport {
    pub dim: usize,
    pub nlist: usize,
    pub nprobe: usize,
    pub metric_type: MetricType,
...
impl IvfUsqIndex {
    ...
    pub fn export_sectioned_snapshot(&self) -> Result<IvfUsqSectionedExport> {
        if !self.trained {
            return Err(KnowhereError::InvalidArg(
                "cannot export untrained IVF-USQ sectioned snapshot".to_string(),
            ));
        }
        validate_ivf_usq_common_config(&self.config)?;
        if self.centroids.len() != self.config.nlist * self.config.dim {
            return Err(KnowhereError::Codec(format!(
                "invalid IVF-USQ state: centroids len {} != nlist * dim {}",
                self.centroids.len(),
        ...
        let padded_dim = usq_config.padded_dim();
        let pending = self.pending.read();

        let mut list_offsets = Vec::with_capacity(self.config.nlist);
        let mut list_sizes = Vec::with_capacity(self.config.nlist);
        let mut list_ids = Vec::with_capacity(self.ntotal);
        let mut packed_bits = Vec::with_capacity(self.ntotal * code_bytes);
        let mut sign_bits = Vec::with_capacity(self.ntotal * sign_bytes);
        let mut norms = Vec::with_capacity(self.ntotal);
        let mut norms_sq = Vec::with_capacity(self.ntotal);
        let mut vmaxs = Vec::with_capacity(self.ntotal);
        let mut quant_qualities = Vec::with_capacity(self.ntotal);
        let mut running = 0u64;

        for cluster in 0..self.config.nlist {
            list_offsets.push(running);
            let entries = pending.get(&cluster).map(Vec::as_slice).unwrap_or(&[]);
            list_sizes.push(entries.len() as u64);
            for entry in entries {
                if entry.encoded.packed_bits.len() != code_bytes {
                    return Err(KnowhereError::Codec(format!(
                        "invalid IVF-USQ state: cluster {cluster} packed_bits len {} != code_bytes {code_bytes}",
                        entry.encoded.packed_bits.len()
                    )));
                }
                if entry.encoded.sign_bits.len() != sign_bytes {
                    return Err(KnowhereError::Codec(format!(
                        "invalid IVF-USQ state: cluster {cluster} sign_bits len {} != sign_bytes {sign_bytes}",
                        entry.encoded.sign_bits.len()
                    )));
                }
                list_ids.push(entry.id);
                packed_bits.extend_from_slice(&entry.encoded.packed_bits);
                sign_bits.extend_from_slice(&entry.encoded.sign_bits);
                norms.push(entry.encoded.norm);
                norms_sq.push(entry.encoded.norm_sq);
                vmaxs.push(entry.encoded.vmax);
                quant_qualities.push(entry.encoded.quant_quality);
            }
            running += entries.len() as u64;
        }

        if list_ids.len() != self.ntotal {
            return Err(KnowhereError::Codec(format!(
                "invalid IVF-USQ state: inverted list count {} != ntotal {}",
                list_ids.len(),
                self.ntotal
            )));
        }

        Ok(IvfUsqSectionedExport {
            dim: self.config.dim,
    ...
    pub fn from_sectioned_snapshot_export(export: IvfUsqSectionedExport) -> Result<Self> {
        validate_ivf_usq_sectioned_export(&export)?;
        ...
        let usq_config = UsqConfig::new(config.dim, config.bits_per_dim as u8)
            .map_err(KnowhereError::InvalidArg)?
            .with_seed(config.rotation_seed);
        ...
        for cluster in 0..export.nlist {
            let offset = export.list_offsets[cluster] as usize;
            let size = export.list_sizes[cluster] as usize;
            if offset != expected_offset {
                return Err(KnowhereError::Codec(format!(
                    "invalid IVF-USQ sectioned snapshot: list {cluster} offset {offset} != expected canonical offset {expected_offset}"
                )));
            }
            let end = offset.checked_add(size).ok_or_else(|| {
                KnowhereError::Codec(format!(
                    "invalid IVF-USQ sectioned snapshot: list {cluster} offset overflow"
                ))
            })?;
            if end > export.list_ids.len() {
                return Err(KnowhereError::Codec(format!(
                    "invalid IVF-USQ sectioned snapshot: list {cluster} range exceeds list_ids len {}",
                    export.list_ids.len()
                )));
            }
            ...
            for absolute in offset..end {
                let packed_start = absolute * export.code_bytes;
                let packed_end = packed_start + export.code_bytes;
                let sign_start = absolute * export.sign_bytes;
                let sign_end = sign_start + export.sign_bytes;
                entries.push(PendingEntry {
                    id: export.list_ids[absolute],
                    encoded: UsqEncoded {
                        packed_bits: export.packed_bits[packed_start..packed_end].to_vec(),
                        sign_bits: export.sign_bits[sign_start..sign_end].to_vec(),
                        norm: export.norms[absolute],
                        norm_sq: export.norms_sq[absolute],
                        vmax: export.vmaxs[absolute],
                        quant_quality: export.quant_qualities[absolute],
                    },
                });
            }
            ...
            if !entries.is_empty() {
                ...
                let encoded: Vec<UsqEncoded> =
                    entries.iter().map(|entry| entry.encoded.clone()).collect();
                let layout = UsqLayout::build(quantizer.config(), &encoded, &ids);
                cluster_layouts.insert(cluster, layout);
                pending.insert(cluster, entries);
            }
            expected_offset = end;
        }
...
fn validate_ivf_usq_common_config(config: &IvfUsqConfig) -> Result<()> {
    if config.dim == 0 || config.nlist == 0 {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ config: dim {} nlist {}",
            config.dim, config.nlist
        )));
    }
    if config.nprobe == 0 {
        return Err(KnowhereError::Codec(
            "invalid IVF-USQ config: nprobe must be > 0".to_string(),
        ));
    }
    if config.bits_per_dim == 0 || config.bits_per_dim > 8 {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ config: bits_per_dim {}",
            config.bits_per_dim
        )));
    }
    Ok(())
...
fn validate_ivf_usq_sectioned_export(export: &IvfUsqSectionedExport) -> Result<()> {
    validate_ivf_usq_common_config(&IvfUsqConfig {
        dim: export.dim,
        nlist: export.nlist,
        nprobe: export.nprobe,
        metric_type: export.metric_type,
    ...
    if !export.trained {
        return Err(KnowhereError::Codec(
            "invalid IVF-USQ sectioned snapshot: trained must be true".to_string(),
        ));
    }
    let usq_config = UsqConfig::new(export.dim, export.bits_per_dim as u8)
        .map_err(KnowhereError::InvalidArg)?
        .with_seed(export.rotation_seed);
    if export.code_bytes != usq_config.code_bytes() {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: code_bytes {} != expected {}",
            export.code_bytes,
            usq_config.code_bytes()
        )));
    }
    if export.sign_bytes != usq_config.sign_bytes() {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: sign_bytes {} != expected {}",
            export.sign_bytes,
            usq_config.sign_bytes()
        )));
    }
    if export.padded_dim != usq_config.padded_dim() {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: padded_dim {} != expected {}",
            export.padded_dim,
            usq_config.padded_dim()
        )));
    }
    let expected_centroids = export.nlist.checked_mul(export.dim).ok_or_else(|| {
        KnowhereError::Codec(
            "invalid IVF-USQ sectioned snapshot: centroid length overflow".to_string(),
        )
    })?;
    if export.centroids.len() != expected_centroids {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: centroids len {} != nlist * dim {}",
            export.centroids.len(),
            expected_centroids
        )));
    }
    if export.quantizer_centroid.len() != export.dim {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: quantizer_centroid len {} != dim {}",
    ...
    if export.list_offsets.len() != export.nlist {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: list_offsets len {} != nlist {}",
    ...
    if export.list_sizes.len() != export.nlist {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: list_sizes len {} != nlist {}",
    ...
    let list_count = export.list_sizes.iter().try_fold(0usize, |sum, &size| {
        sum.checked_add(size as usize).ok_or_else(|| {
            KnowhereError::Codec(
                "invalid IVF-USQ sectioned snapshot: list size overflow".to_string(),
            )
        })
    })?;
    if list_count != export.ntotal || export.list_ids.len() != export.ntotal {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: list count {list_count} ids len {} ntotal {}",
            export.list_ids.len(),
            export.ntotal
        )));
    }
    let expected_packed = export
        .ntotal
        .checked_mul(export.code_bytes)
        .ok_or_else(|| {
            KnowhereError::Codec(
                "invalid IVF-USQ sectioned snapshot: packed length overflow".to_string(),
            )
        })?;
    if export.packed_bits.len() != expected_packed {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: packed_bits len {} != ntotal * code_bytes {}",
            export.packed_bits.len(),
            expected_packed
        )));
    }
    let expected_sign = export
        .ntotal
        .checked_mul(export.sign_bytes)
        .ok_or_else(|| {
            KnowhereError::Codec(
                "invalid IVF-USQ sectioned snapshot: sign length overflow".to_string(),
            )
        })?;
    if export.sign_bits.len() != expected_sign {
        return Err(KnowhereError::Codec(format!(
            "invalid IVF-USQ sectioned snapshot: sign_bits len {} != ntotal * sign_bytes {}",
            export.sign_bits.len(),
            expected_sign
        )));
    }
    for (name, len) in [
        ("norms", export.norms.len()),
        ("norms_sq", export.norms_sq.len()),
        ("vmaxs", export.vmaxs.len()),
        ("quant_qualities", export.quant_qualities.len()),
    ] {
        if len != export.ntotal {
            return Err(KnowhereError::Codec(format!(
                "invalid IVF-USQ sectioned snapshot: {name} len {len} != ntotal {}",
                export.ntotal
            )));
        }
    }
