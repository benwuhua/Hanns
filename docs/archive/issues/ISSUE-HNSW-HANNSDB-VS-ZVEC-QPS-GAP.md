# ISSUE: HannsDB (hanns backend) vs zvec QPS 差距
...
| 测量场景 | 数据 | M | p50 | p99 |
|---------|------|---|-----|-----|
| hanns 原生 harness（SCRATCH-001 等修复后）| 合成随机 | 8 | 3.021ms | 3.508ms |
| hanns 原生 harness（SCRATCH-001 等修复后）| 合成随机 | 16 | 5.456ms | 6.073ms |
...
| **hanns AVX512** | `.so` 包含 440 条 zmm/avx512 指令；源码有 `#[target_feature(enable="avx512f")]` + runtime `is_x86_feature_detected!`；CPU 支持 avx512f/bw/vl |
...
hanns 编译产物含 440 条 AVX512 指令，runtime dispatch 正常工作，CPU 支持。两者均使用 AVX512。
...
1. **Early termination / 自适应截断**：zvec mean=0.36ms << p99=0.90ms（双峰分布），说明大多数查询在 ~0.3ms 提前收敛，偶尔触发慢路径。hanns mean≈p99=0.65ms（单峰收敛），表明**每次查询都走完整 ef 轮遍历，无早停**。
