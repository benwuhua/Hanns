...
所有修复 + 优化（x86 权威数字，1000 queries，50 warmup）：

### 修复阶段对比（50K/1536/cosine/ef=32）

| 阶段 | p99 | vs 原始 |
|------|-----|---------|
| 修复前（原始 HannsDB）| 110ms | baseline |
| SCRATCH-001 + QNORM-001 (94ca617) | 8.766ms | **-92% (12.5x)** |
| + pre-normalize vectors (c284d2c) | 6.073ms (M=16) | **-94% (18x)** |
| **+ M=8 配置 (9daa85d)** | **3.508ms** | **-97% (31x)** |

### M=8 vs M=16 x86 权威对比（含 pre-normalize，2026-03-23）

| 配置 | M=8 p50 | M=8 p99 | M=16 p50 | M=16 p99 |
|------|---------|---------|---------|---------|
| 10K/768/ef=32 | 985µs | 1019µs | 1360µs | 1406µs |
| 50K/768/ef=32 | 1648µs | 1897µs | 2795µs | 7817µs |
| 10K/1536/ef=32 | 1589µs | 1657µs | 2366µs | 2436µs |
| **50K/1536/ef=32** | **3021µs** | **3508µs** | **5456µs** | **6073µs** |

M=8 vs M=16：p50 约 1.4-1.8x 更快，p99 显著改善（50K/768: 1897µs vs 7817µs）。
M=8 build 时间也更短（50K/1536: 106s vs 353s）。

**剩余差距**: 目标 zvec p99=0.6ms；当前 M=8 p50=3.021ms（仍有 ~5x 差距）。
根本原因待查：zvec 可能使用更小 M（M=4 或更低），或 fp16/int8 量化存储（2x 内存带宽优势）。
注：合成随机数据 recall 未测，M=8 recall 会低于 M=16，实际生产需用真实数据验证。
