...
- 2026-03-14: **builder-loop：完成 `hnsw-round12-authority-same-schema-rerun`，Rust authority lane 成功跑完但以 hard stop 收口（authority/hard_stop）**
  ...
  3. 本轮执行：监控 `/data/work/hanns-logs-hnsw-reopen-round12/test_20260314T060359Z_20504.status` 直到 `status=ok`，随后把 fresh `benchmark_results/rs_hnsw_sift128.full_k100.json` 拉回本地，拿到 Rust `1497.689` qps / recall `0.9942`；再通过 linkfix fallback 运行 native benchmark，得到 `/data/work/hanns-logs-hnsw-reopen-round12/native_hnsw_qps_linkfix_20260314T062053Z.log` 中的 trusted native row `8502.128` qps / recall `0.95`。最后重新收紧 round-12 default-lane contract，新增 `benchmark_results/hnsw_reopen_round12_authority_summary.json`，并回放 remote default-lane replay。
...
- 2026-03-14: **builder-loop：尝试执行 `hnsw-round12-authority-same-schema-rerun`，但 authority 代理链路在 benchmark 过程中反复 reset（authority/blocked）**
  ...
  3. 本轮执行：`bash init.sh` 成功，说明 authority bootstrap 在起跑时是正常的；随后把隔离的 round-12 Rust same-schema benchmark 发到 `/data/work/hanns-target-hnsw-reopen-round12` / `/data/work/hanns-logs-hnsw-reopen-round12`。但在等待远端结果时，SSH/proxy 控制链路被 `socks5 proxy error: [Errno 54] Connection reset by peer` 直接切断；紧接着的极小 `run_ssh "echo remote_ok"` 探针也复现了同样的 reset。
...
- 2026-03-14: **builder-loop：补跑并关闭 `hnsw-reopen-round12-activation`，authority bootstrap 与 remote default-lane replay 均通过（activation/passing）**
  ...
  3. 本轮执行：重新运行 `bash init.sh`，确认 workspace 已同步到 remote x86 且 toolchain probe 正常；随后使用隔离的 `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round12` 与 `HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round12` 回放 `cargo test --test bench_hnsw_reopen_round12 -q`，验证新的 round-12 baseline artifact 与被有意忽略的 future authority-summary contract 一并在 authority 路径上存活。
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round12 -- --nocapture` 继续通过；`bash init.sh` 成功完成远端同步并打印 authority ready；remote default-lane replay 返回 `test=ok`，日志为 `/data/work/hanns-logs-hnsw-reopen-round12/test_20260314T055626Z_16735.log`；因此 `hnsw-reopen-round12-activation` 现在可以诚实地标成 `passing`。
...
- 2026-03-13: **builder-loop：收口 `hnsw-round11-authority-same-schema-rerun`，给 filtered brute-force fallback 做 authority 终判（exec）**
  ...
  4. 验证结果：authority Rust same-schema rerun `/data/work/hanns-logs-hnsw-reopen-round11/test_20260313T082501Z_10580.log` 失败并以 `exit_code=143` 结束；native capture `/data/work/hanns-logs-hnsw-reopen-round11/native_hnsw_qps_linkfix_20260313T082518Z.log` 成功并给出 `8897.304` qps；本地 `cargo test --test bench_hnsw_reopen_round11 -- --nocapture` 通过；authority default-lane replay 首次命中 stale remote target cache 失败，清理 `/data/work/hanns-target-hnsw-reopen-round11` 后重放通过，最终日志 `/data/work/hanns-logs-hnsw-reopen-round11/test_20260313T084627Z_14485.log`；`python3 scripts/validate_features.py feature-list.json` 通过。
...
- 2026-03-13: **builder-loop：重开 `hnsw-reopen-round11-activation`，把下一条 HNSW tracked hypothesis 切到 filtered brute-force fallback（screen promote -> tracked activation）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round11 -- --nocapture` 按预期先因缺失 baseline artifact 失败，随后通过 activation baseline 更新恢复为默认-lane可接受状态；`bash init.sh` 通过；authority default-lane replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round11 HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round11 bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_reopen_round11 -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-round11/test_20260313T082058Z_9460.log`；`python3 scripts/validate_features.py feature-list.json` 返回 `VALID - 64 features (63 passing, 1 failing); workflow/doc checks passed`。
...
- 2026-03-13: **builder-loop：收口 `hnsw-layer0-slab-audit-round10` 与 `hnsw-round10-authority-same-schema-rerun`，给 round10 locality hypothesis 做 authority 终判（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test hnsw --lib -- --nocapture`、`cargo test --features long-tests --test bench_hnsw_reopen_round10_profile -- --ignored --nocapture`、`cargo test --test bench_hnsw_reopen_round10 -- --nocapture`、`cargo fmt --all -- --check` 均通过；authority Rust same-schema HDF5 日志 `/data/work/hanns-logs-hnsw-reopen-round10/test_20260313T061915Z_87617.log` 通过，得到 `1831.504` qps / `0.9957` recall；fresh native capture 日志 `/data/work/hanns-logs-hnsw-reopen-round10/native_hnsw_qps_linkfix_20260313T061934Z.log` 通过，得到 `8900.882` qps / `0.95` recall；authority profile replay 日志 `/data/work/hanns-logs-hnsw-reopen-round10/test_20260313T063647Z_92534.log` 通过；authority default-lane replay 日志 `/data/work/hanns-logs-hnsw-reopen-round10/test_20260313T063939Z_93275.log` 通过。
...
- 2026-03-13: **builder-loop：收口 `hnsw-search-fastpath-audit-round9` 与 `hnsw-round9-authority-same-schema-rerun`，把 round9 production fast-path hypothesis 跑到 authority 终局（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test hnsw --lib -- --nocapture`、`cargo test --features long-tests --test bench_hnsw_reopen_round9_profile -- --ignored --nocapture`、`cargo test --test bench_hnsw_reopen_round9 -- --nocapture` 均通过，`cargo fmt --all -- --check` 先红后绿；authority Rust same-schema HDF5 日志 `/data/work/hanns-logs-hnsw-reopen-round9/test_20260313T034306Z_66062.log` 通过，得到 `1845.608` qps / `0.9909` recall；fresh native capture 日志 `/data/work/hanns-logs-hnsw-reopen-round9/native_hnsw_qps_linkfix_20260313T034322Z.log` 通过，得到 `10348.740` qps / `0.95` recall；authority profile replay 首次因未 resync 新 test target 失败，再经 `bash init.sh` 重放后通过，最终日志 `/data/work/hanns-logs-hnsw-reopen-round9/test_20260313T040153Z_74459.log`；authority default-lane replay `/data/work/hanns-logs-hnsw-reopen-round9/test_20260313T040406Z_75601.log` 通过。
...
- 2026-03-13: **builder-loop：收口 `hnsw-round8-authority-same-schema-rerun`，用 fresh authority same-schema evidence 给 round 8 graph-quality hypothesis 终判（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round8 -- --nocapture` 先因缺失 round-8 summary artifact 失败再转绿；`bash init.sh` 通过；authority Rust same-schema HDF5 日志 `/data/work/hanns-logs-hnsw-reopen-round8/test_20260313T015649Z_53053.log` 通过并回传新的 `benchmark_results/rs_hnsw_sift128.full_k100.json`；fresh native capture 日志 `/data/work/hanns-logs-hnsw-reopen-round8/native_hnsw_qps_linkfix_20260313T015830Z.log` 通过；authority profile replay 日志 `/data/work/hanns-logs-hnsw-reopen-round8/test_20260313T021527Z_55615.log` 通过；authority default-lane replay 日志 `/data/work/hanns-logs-hnsw-reopen-round8/test_20260313T021527Z_55616.log` 通过。
...
- 2026-03-13: **builder-loop：收口 `hnsw-parallel-build-graph-audit-round8`，把 bulk-build graph-quality 偏差锁成 authority-backed audit artifact（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --features long-tests --test bench_hnsw_reopen_round8_profile -- --ignored --nocapture` 先因缺失 `HnswParallelBuildProfileReport` / `parallel_build_profile_report` 失败再转绿；`cargo fmt --all -- --check` 先红后绿；本地 `cargo test --test bench_hnsw_reopen_round8 -- --nocapture` 通过；`bash init.sh` 通过；authority profile replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round8 HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round8 bash scripts/remote/test.sh --command "cargo test --features long-tests --test bench_hnsw_reopen_round8_profile -- --ignored --nocapture"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-round8/test_20260313T012954Z_47513.log`；authority contract replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round8 HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round8 bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_reopen_round8 -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-round8/test_20260313T013023Z_47680.log`。
...
- 2026-03-13: **builder-loop：收口 `hnsw-reopen-round8-activation`，把 round 8 baseline 和默认-lane contract 正式挂回 durable workflow（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round8 -- --nocapture` 先因缺失 baseline artifact 失败再转绿；`bash init.sh` 通过；authority replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round8 HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round8 bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_reopen_round8 -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-round8/test_20260313T011357Z_43949.log`；`python3 scripts/validate_features.py feature-list.json` 返回 `VALID - 56 features (53 passing, 3 failing); workflow/doc checks passed`。
...
- 2026-03-12 12:22: **builder-loop：收口 `hnsw-round4-authority-same-schema-rerun`，给第四轮 HNSW reopen 做真实 authority same-schema 终判（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round4 -- --nocapture` 先因缺失 `benchmark_results/hnsw_reopen_round4_authority_summary.json` 失败再转绿；`bash init.sh` 通过；authority Rust same-schema rerun `generate_hdf5_hnsw_baseline` 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T115946Z_42625.log`；native capture 通过 linkfix fallback，日志 `/data/work/hanns-logs-hnsw-reopen-round4/native_hnsw_qps_linkfix_20260312T121538Z.log`；authority round-4 synthetic profile rerun 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T121650Z_45259.log`；summary artifact 回推远端后，authority contract replay `bench_hnsw_reopen_round4 -q` 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T122210Z_46060.log`。
...
- 2026-03-12 11:52: **builder-loop：收口 `hnsw-layer0-searcher-core-rework`，把 round-4 layer-0 parity 假设落成真正的搜索核心改动（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test hnsw --lib -- --nocapture` 先因缺失 ordered-pool helpers 失败再转绿；本地 `cargo test --test bench_hnsw_reopen_round4 -- --nocapture` 先因 stale audit artifact 仍写着 `dual_binary_heap` 失败再转绿；本地 `cargo test --test bench_hnsw_cpp_compare -q`、`cargo test --test bench_hnsw_reopen_round3 -q`、`cargo test --features long-tests --test bench_hnsw_reopen_round4_profile -- --ignored --nocapture`、`cargo fmt --all -- --check` 均通过；`bash init.sh` 通过；authority compare replay 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T115024Z_39686.log`；authority long-test profile replay 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T115054Z_39863.log`；authority default-lane contract replay 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T115136Z_40082.log`。
...
- 2026-03-12 11:35: **builder-loop：收口 `hnsw-layer0-searcher-audit`，把第四轮 HNSW 的 native-vs-Rust layer-0 结构差异锁成 authority-backed artifact（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round4 -- --nocapture` 先因缺失 `benchmark_results/hnsw_reopen_layer0_searcher_audit_round4.json` 失败再转绿；本地 `cargo test --features long-tests --test bench_hnsw_reopen_round4_profile -- --ignored --nocapture` 通过；`cargo fmt --all -- --check` 先红后绿；`bash init.sh` 通过；authority round-4 long-test replay 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T113413Z_36164.log`；authority round-4 contract replay 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T113455Z_36352.log`。
...
- 2026-03-12 11:22: **builder-loop：收口 `hnsw-reopen-round4-activation`，把第四轮 HNSW layer-0 parity 攻关线正式挂回 durable workflow（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round4 -- --nocapture` 先红后绿；`bash init.sh` 通过；首条 authority replay 因在同步完成前启动而失败，不作为最终证据（`/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T112127Z_32612.log`）；串行 authority replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round4 HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round4 bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_reopen_round4 -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-round4/test_20260312T112200Z_32809.log`。
...
- 2026-03-12 09:56: **builder-loop：收口 `hnsw-round3-authority-same-schema-rerun`，给第三轮 HNSW reopen 做真实 authority same-schema 终判（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round3 -- --nocapture` 先因缺失 `benchmark_results/hnsw_reopen_round3_authority_summary.json` 失败再转绿；`bash init.sh` 通过；authority Rust same-schema rerun `generate_hdf5_hnsw_baseline` 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round3/test_20260312T093400Z_21869.log`；native capture 通过 linkfix fallback，日志 `/data/work/hanns-logs-hnsw-reopen-round3/native_hnsw_qps_linkfix_20260312T094945Z.log`；authority round-3 synthetic profile rerun 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round3/test_20260312T095112Z_24204.log`；summary artifact 同步回 remote 后，authority contract replay `bench_hnsw_reopen_round3 -q` 通过，日志 `/data/work/hanns-logs-hnsw-reopen-round3/test_20260312T095502Z_24753.log`。
...
- 2026-03-12 09:13: **builder-loop：收口 `hnsw-distance-compute-profiler`，把第三轮 HNSW 的 `distance_compute` 再拆成 authority-backed 子热点（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round3 -- --nocapture` 先红后绿；本地 `cargo test --features long-tests --test bench_hnsw_reopen_round3_profile -- --ignored --nocapture` 通过；`cargo fmt --all -- --check` 先红后绿；`bash init.sh` 通过；第一次并行 authority contract replay 因 shared wrapper lock 返回 `status=conflict`（`/data/work/hanns-logs-hnsw-reopen-round3/test_20260312T091236Z_18116.log`），最终有效证据来自串行成功 reruns：`bench_hnsw_reopen_round3_profile` 日志 `/data/work/hanns-logs-hnsw-reopen-round3/test_20260312T091236Z_18115.log`，`bench_hnsw_reopen_round3` 日志 `/data/work/hanns-logs-hnsw-reopen-round3/test_20260312T091319Z_18338.log`；`python3 scripts/validate_features.py feature-list.json` 返回 `VALID - 43 features (41 passing, 2 failing); workflow/doc checks passed`。
...
- 2026-03-12 09:03: **builder-loop：收口 `hnsw-reopen-round3-activation`，把第三轮 HNSW 的 distance-compute 线正式挂回 durable workflow（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round3 -- --nocapture` 先红后绿；`bash init.sh` 通过；第一次 authority replay 因为我把 replay 和 `init.sh` 并行跑而不算最终证据，串行 replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round3 HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round3 bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_reopen_round3 -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-round3/test_20260312T090426Z_16155.log`；`python3 scripts/validate_features.py feature-list.json` 返回 `VALID - 43 features (40 passing, 3 failing); workflow/doc checks passed`。
...
- 2026-03-12 08:45: **builder-loop：收口 `hnsw-round2-authority-same-schema-rerun`，用 fresh authority same-schema evidence 给第二轮 HNSW reopen 终判（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round2 -- --nocapture` 先红后绿；`cargo fmt --all -- --check` 先红后绿；authority Rust same-schema HDF5 日志为 `/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T082542Z_10202.log`；native fresh capture 日志为 `/data/work/hanns-logs-hnsw-reopen-round2/native_hnsw_qps_linkfix_20260312T083313Z.log`；authority round-2 profile refresh 日志为 `/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T084128Z_12238.log`；第一次 authority contract replay 按预期因为 summary 尚未同步而失败（`/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T084211Z_12364.log`），同步后最终 replay 转绿（`/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T084531Z_12900.log`）；`python3 scripts/validate_features.py feature-list.json` 返回 `VALID - 39 features (39 passing, 0 failing); workflow/doc checks passed`。
...
- 2026-03-12 08:18: **builder-loop：收口 `hnsw-candidate-search-core-rework`，把第二轮 HNSW 的 shared candidate-search core 真正改掉（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test hnsw --lib -- --nocapture` 先因缺失 helper 报错再转绿；本地 `cargo test --test bench_hnsw_cpp_compare -q`、`cargo test --test bench_hnsw_reopen_round2 -q` 通过；`cargo fmt --all -- --check` 先红后绿；`bash init.sh` 通过；第一次并行 authority 回放中一条 lane 因 shared wrapper lock 返回 `status=conflict`（`/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T081711Z_8977.log`），最终有效证据来自串行成功 reruns：`bench_hnsw_cpp_compare` 日志 `/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T081711Z_8978.log`，`bench_hnsw_reopen_round2` 日志 `/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T081736Z_9088.log`；`python3 scripts/validate_features.py feature-list.json` 返回 `VALID - 39 features (38 passing, 1 failing); workflow/doc checks passed`。
...
- 2026-03-12 08:09: **builder-loop：收口 `hnsw-candidate-search-profiler`，把第二轮 HNSW 的 `candidate_search` 大桶拆成 authority-backed 热点图（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round2 -- --nocapture` 先红后绿；本地 `cargo test --features long-tests --test bench_hnsw_reopen_round2_profile -- --ignored --nocapture` 通过；`bash init.sh` 通过；authority long-test replay 日志为 `/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T080534Z_6832.log`；authority default-lane replay 日志为 `/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T080615Z_7017.log`；`python3 scripts/validate_features.py feature-list.json` 通过并返回 `VALID - 39 features (37 passing, 2 failing); workflow/doc checks passed`。
...
- 2026-03-12 15:54: **builder-loop：收口 `hnsw-reopen-round2-activation`，把 HNSW 第二轮 candidate-search 攻关线正式挂回 durable workflow（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_round2 -- --nocapture` 先红后绿；`bash init.sh` 通过；authority replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-round2 HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-round2 bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_reopen_round2 -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-round2/test_20260312T075654Z_4565.log`；`python3 scripts/validate_features.py feature-list.json` 返回 `VALID - 39 features (36 passing, 3 failing); workflow/doc checks passed`。
...
- 2026-03-12 15:28: **builder-loop：收口 `hnsw-authority-rerun-and-verdict-refresh`，刷新第一轮 HNSW reopen authority 结果并确认历史 verdict 暂不改写（plan+exec）**
  ...
  4. 验证结果：`bash init.sh` 通过；authority `bench_hnsw_reopen_profile` 日志为 `/data/work/hanns-logs-hnsw-reopen/test_20260312T072331Z_99668.log`；最终串行 authority compare/progress replays 日志为 `/data/work/hanns-logs-hnsw-reopen/test_20260312T072549Z_118.log` 与 `/data/work/hanns-logs-hnsw-reopen/test_20260312T072619Z_220.log`；本地 `cargo test --test bench_hnsw_cpp_compare -q`、`cargo test --test bench_hnsw_reopen_progress -q` 与 `python3 scripts/validate_features.py feature-list.json` 均通过。
...
- 2026-03-12 07:22: **builder-loop：收口 `hnsw-build-quality-rework`，交付第一刀真实 HNSW 算法改动（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test hnsw --lib -- --nocapture` 先红后绿；focused `test_parallel_bulk_build_matches_single_insert_layer0_neighbor_diversification` 和 `test_hnsw_parallel_build` 都通过；`cargo test --test bench_hnsw_reopen_progress -q` 与 `cargo test --test bench_hnsw_cpp_compare -q` 通过；authority safety replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-rework HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-rework bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_cpp_compare -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-rework/test_20260312T071858Z_98869.log`。
...
- 2026-03-12 07:05: **builder-loop：收口 `hnsw-build-path-profiler`，把 HNSW reopen line 从“只有基线”推进到“有可执行热点分解”的状态（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_progress -- --nocapture` 先因缺失 profile artifact 失败后转绿；`cargo test --features long-tests --test bench_hnsw_reopen_profile -- --ignored --nocapture` 通过并重写 artifact；authority replay 在重跑 `bash init.sh` 刷新远端同步后成功，日志分别为 `/data/work/hanns-logs-hnsw-reopen-profiler/test_20260312T070256Z_96556.log` 与 `/data/work/hanns-logs-hnsw-reopen-profiler/test_20260312T070349Z_96795.log`。
...
- 2026-03-12 06:47: **builder-loop：重开 `hnsw-reopen-baseline-freeze`，把 HNSW 从 archived verdict 重新切回 active algorithm line（plan+exec）**
  ...
  4. 验证结果：本地 `cargo test --test bench_hnsw_reopen_progress -- --nocapture` 先红后绿；`bash init.sh` 成功；authority replay `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-hnsw-reopen-baseline HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-hnsw-reopen-baseline bash scripts/remote/test.sh --command "cargo test --test bench_hnsw_reopen_progress -q"` 返回 `test=ok`，日志 `/data/work/hanns-logs-hnsw-reopen-baseline/test_20260312T064856Z_93162.log`。
...
- 2026-03-12 05:36: **builder-loop：收口 `prod-ffi-observability-persistence-gate`，关闭 authority FFI/serialize/JSON export gate（plan+exec）**
  ...
  3. 本轮执行：先在本地重新确认 `cargo test --lib ffi -- --nocapture`、`cargo test --lib serialize -- --nocapture`、`cargo test --test bench_json_export -q` 全部为绿；随后在 isolated `HANNS_REMOTE_TARGET_DIR=/data/work/hanns-target-prod-ffi-observability-persistence-gate` 与 `HANNS_REMOTE_LOG_DIR=/data/work/hanns-logs-prod-ffi-observability-persistence-gate` 下串行重放三条 authority tests。初始并行尝试触发了 wrapper shared-lock `status=conflict`，因此最终 authority evidence 以串行 isolated runs 为准。
...
- 2026-03-07 11:55: **BUG-P1-001 子项修复：IVF-CC 检索候选不足导致 topk 回归失败 + 双模块复核（src/index.rs / src/ffi.rs）**
  1. 执行同步与基线更新：`git fetch origin && git checkout main && git pull origin main && git rev-parse origin/main` -> `4f60908fc9ad7438b4b8ff64210481ab281009b0`
  2. C++/Rust 增量扫描：`find .../knowhere/src/index -maxdepth 2 -type f | wc -l`=49，`find .../include/knowhere/index -maxdepth 2 -type f | wc -l`=8，`find .../hanns/src/faiss -maxdepth 1 -type f | wc -l`=47
  3. 逐接口复核：
     - `src/index.rs`：`Index` trait 仍提供 `create_ann_iterator/get_vector_by_ids/save/load` 统一入口
...
- 2026-03-07 11:00: **FFI index_type 返回表补齐（多索引）+ 双模块复核（src/index.rs / src/ffi.rs）**
  ...
  2. C++/Rust 增量扫描：`find .../knowhere/src/index -maxdepth 1 -type f | wc -l`=7，`find .../include/knowhere/index -maxdepth 1 -type f | wc -l`=8，`find .../hanns/src/faiss -maxdepth 1 -type f | wc -l`=47
  3. 逐接口复核：
...
- 2026-03-06 23:58: **BUG-P0-004 编译回归收敛 + 双模块复核（src/index.rs / src/ffi.rs）**
  ...
  2. C++/Rust 增量扫描：`ls .../knowhere/src/index | wc -l`=17，`ls .../include/knowhere/index | wc -l`=8，`ls .../hanns/src/faiss | wc -l`=47
  ...
  4. 回归修复：批量修复 tests/examples 的 `IndexConfig::data_type` 缺失；统一 `crate::api::DataType` -> `hanns::api::DataType`
  5. 验证结果：
...
- 2026-03-06 18:02: **MinHash Index trait wrapper 接入 + 全量测试回归诊断**
  1. 执行同步与基线更新：`git fetch origin && git checkout main && git pull origin main && git rev-parse origin/main` -> `388aea90260084965f965b29e0a8b87b7a808d51`
  2. 对齐扫描命令：`ls /Users/ryan/Code/vectorDB/knowhere/src/index && ls /Users/ryan/Code/vectorDB/knowhere/include/knowhere/index && ls /Users/ryan/.openclaw/workspace-builder/hanns/src/faiss`
...
- 2026-03-06 14:38: **MinHash FFI 查询长度对齐修复 + 抽样复核**
  1. 执行同步与基线更新：`git fetch origin && git checkout main && git pull origin main && git rev-parse origin/main` -> `388aea90260084965f965b29e0a8b87b7a808d51`
  2. 抽样深比对命令：`ls /Users/ryan/Code/vectorDB/knowhere/src/index && ls /Users/ryan/Code/vectorDB/knowhere/include/knowhere/index && ls /Users/ryan/.openclaw/workspace-builder/hanns/src/faiss`
...
- 2026-03-05 22:32: Detailed interface comparison between C++ and Rust.
  ...
  - **Files checked:**
    ...
    - Rust: `/Users/ryan/.openclaw/workspace-builder/hanns/src/index.rs:102-267`
