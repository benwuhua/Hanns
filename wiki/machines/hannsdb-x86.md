...
| 用途 | 路径 |
|------|------|
| knowhere-rs 源码 | `/data/work/milvus-rs-integ/knowhere-rs/` |
| knowhere-rs build cache | `/data/work/milvus-rs-integ/knowhere-rs-target/` |
...
ssh hannsdb-x86 'cd /data/work/milvus-rs-integ && \
  ...
  nohup bash /data/work/milvus-rs-integ/milvus-src/scripts/knowhere-rs-shim/start_standalone_remote.sh \
...
- **rsync 务必加 `--exclude='data/'`**，否则会删除 SIFT-1M 数据（路径 `/data/work/knowhere-rs-src/data/sift`，实际在 `/data/work/datasets/sift-1m/`）
